...
class MongoDBSessionStorage {
  ...
  async findSessionsByShop(shop) {
    await this.ready;
    const rawResults = await this.collection.find({
      shop
    }).toArray();
    if (!rawResults || (rawResults === null || rawResults === void 0 ? void 0 : rawResults.length) === 0) return [];
  ...
  get collection() {
    return this.client.db(this.dbName).collection(this.options.sessionCollectionName);
  }
  async init() {
  ...
  async hasSessionCollection() {
    ...
    return collections.map(collection => collection.collectionName).includes(this.options.sessionCollectionName);
  }
